...
module.exports = async function handler(req, res) {
    ...
    if (token) {
        ...
        if ((count || 0) >= GUEST_HOURLY_LIMIT) {
            return res.status(429).json({ error: 'Límite de análisis sin cuenta alcanzado por ahora. Crea una cuenta gratis para seguir usando la precisión de IA.' });
        }
...
`.trim();
    ...
    const metricsText = metrics ? (
        ...
        '- Contraste fondo/tinta (fórmula WCAG): ' + metrics.contrast + ':1\n' +
        '- Simetría respecto al eje vertical: ' + metrics.symmetryScore + '%\n' +
        '- Complejidad de forma (densidad de bordes): ' + metrics.edgeComplexity + '/100\n' +
        '- Unidades gráficas (componentes conectados) detectadas: ' + metrics.componentCount + '\n' +
        '- Colores en la paleta: ' + metrics.colorCount + '\n' +
        '- Cobertura de tinta sobre el encuadre: ' + Math.round((metrics.inkRatio || 0) * 100) + '%'
    ) : '';

    const contextText = context && (context.brandName || context.sector || context.competitors || context.attributes) ? (
        'Contexto proporcionado por el usuario:\n' +
        '- Nombre de marca: ' + (context.brandName || 'no especificado') + '\n' +
        '- Sector: ' + (context.sector || 'no especificado') + '\n' +
        '- Competencia: ' + (context.competitors || 'no especificada') + '\n' +
        '- Atributos de identidad: ' + (context.attributes || 'no especificados')
    ) : '';

    const systemPrompt = 'Eres un experto en diseño gráfico y gestión de marca, formado en el marco de indicadores de calidad de Norberto Chaves y Raúl Belluccia. Analizas marcas gráficas (logotipos, símbolos, combinaciones) con el mismo rigor con el que un diseñador senior evaluaría el trabajo de otro.\n\n' +
    ...
    const userText = 'Analiza esta marca gráfica.\n\n' + metricsText + (contextText ? '\n\n' + contextText : '');

    const tool = {
        name: 'submit_brand_analysis',
        description: 'Envía el análisis estructurado y completo de la marca gráfica evaluada.',
        input_schema: {
            type: 'object',
            properties: {
                typology: { type: 'string', enum: ['logotipo_puro', 'logotipo_con_fondo', 'logotipo_con_accesorio', 'logo_simbolo', 'logotipo_con_simbolo', 'simbolo_solo'] },
                ...
                diagnostic_summary: { type: 'string', description: 'Párrafo de diagnóstico general en tono profesional de diseño, 3 a 5 frases.' }
            },
            required: ['typology', 'typology_confidence', 'typology_justification', 'calidad_grafica', 'calidad_grafica_justification', 'reproducibilidad', 'reproducibilidad_justification', 'legibilidad', 'legibilidad_justification', 'inteligibilidad', 'inteligibilidad_justification', 'vocatividad', 'vocatividad_justification', 'pregnancia', 'pregnancia_justification', 'diagnostic_summary']
        }
    };
    ...
    try {
        ...
        if (!response.ok) {
            return res.status(response.status).json({ error: (data.error && data.error.message) || 'Error llamando a Claude' });
        }

        const toolUse = (data.content || []).find(b => b.type === 'tool_use');
        if (!toolUse) {
            return res.status(502).json({ error: 'Claude no devolvió el análisis estructurado esperado' });
        }
